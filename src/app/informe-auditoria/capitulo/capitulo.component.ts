...
import { InformePreviewComponent } from '../preview/informe/informe-preview.component';
...
export class CapituloComponent implements OnInit {
  @ViewChild('informePreview') informePreview!: InformePreviewComponent;
  informeId!: number;
  informe!: InformeDTO;
  ...
  async previewInforme(): Promise<void> {
    // 0) Cerrar modal y limpiar previsualizaciones previas
    if (Swal.isVisible()) Swal.close();
    document
      .querySelectorAll('[id^="preview-zoom"], style.swal2-previsualizacion-a4, .swal2-container')
      .forEach(el => el.remove());
    this.loading = true;

    // 1) Cabecera
    const encabezados = await firstValueFrom(this.encabezadoService.getByInformeId(this.informe.id));
    ...
    const capítulosConSecciones = await Promise.all(
      capitulosOrdenados.map(async cap => {
        ...
        const tablasPorSeccion = await Promise.all(
          seccionesDTO.map(s => firstValueFrom(this.tablaService.getTablasPorSeccion(s.id!)))
        );
        return { cap, secciones: seccionesDTO, tablasPorSeccion };
      })
    );

    // 3) Pre-cargar imágenes
    const imageUrls: string[] = [];
    if (logoIzq) imageUrls.push(logoIzq);
    if (logoDer) imageUrls.push(logoDer);
    capítulosConSecciones.forEach(item =>
      item.secciones.forEach(sec =>
        sec.adjuntos?.forEach(a => a.urlAdjunto && imageUrls.push(a.urlAdjunto))
      )
    );
    await this.preloadImages(imageUrls);

    // 4) Medir A4 (297 mm – 20 mm) en px
    const pxPorMm = (mm: number) => (mm / 25.4) * 96;
    const pageHeightPx = pxPorMm(297 - 20);
  ...
  `;
    ...
    // 6) Paginación dinámica “Word-like”
    const pages: string[] = [];
    ...
    for (const { cap, secciones, tablasPorSeccion } of capítulosConSecciones) {
      // —Título de capítulo—
      const titleBlock = `<div class="section-block"><h3>${cap.titulo}</h3></div>`;
      measurer.innerHTML = titleBlock;
      const hTitle = (measurer.firstElementChild as HTMLElement).offsetHeight;

      // Salto solo si NO cabe; si solo está header, NO generar página en blanco
      if (usedHeight + hTitle > pageHeightPx && currentHtml.trim() !== headerHtml.trim()) {
      ...
      // —Secciones, tablas e imágenes—
      secciones.forEach((sec, i) => {
        const lines = sec.contenido.split('\n').map(l => l.trim()).filter(l => l);
        let contentHtml: string;
        switch (sec.styleTemplateNombre) {
          case 'titulo':
            contentHtml = `<div style="font-size:19px;font-weight:bold;text-decoration:underline;margin:0 0 .5em 0;">${sec.contenido}</div>`;
            break;
          case 'subtitulo':
            contentHtml = `<div style="font-size:16px;font-weight:bold;margin:0 0 .4em 0;">${sec.contenido}</div>`;
            break;
          case 'viñeta':
            if (lines.length < 2) contentHtml = `<p>${sec.contenido}</p>`;
            else {
              contentHtml = `<ul style="display:table;margin:0 auto 1em;padding-left:8mm;list-style-type:disc;list-style-position:outside;">${lines
                .map(item => `<li style="font-size:18px;line-height:1.2;padding-left:2mm;text-indent:-2mm;margin:0 0 .1em 0;"><span style="font-size:12px;">${item}</span></li>`)
                .join('')}</ul>`;
            }
            break;
          default:
        ...
        const tablasHtml = tablasPorSeccion[i]
          .map(t => `<div class="tabla-titulo">${t.nombre}</div><table><tbody>${t.filas
            .map(f => `<tr>${f.celdas.map(c => `<td>${c.contenido}</td>`).join('')}</tr>`)
            .join('')}</tbody></table>`)
          .join('');

        const imgsCount = sec.adjuntos?.length ?? 0;
        const cols = imgsCount > 1 ? `repeat(${imgsCount},1fr)` : '1fr';
        const block = `<div class="section-block">${contentHtml}<div class="images-wrapper" style="display:grid;grid-template-columns:${cols};gap:5mm;margin-bottom:1em;">${sec.adjuntos
          ?.map(a => `<img src="${a.urlAdjunto}" style="width:100%;height:auto;object-fit:cover;border:1px solid #ccc;border-radius:4px;"/>`)
          .join('')}</div>${tablasHtml}</div>`;

        measurer.innerHTML = block;
        const bh = (measurer.firstElementChild as HTMLElement).offsetHeight;

        if (usedHeight + bh > pageHeightPx && currentHtml.trim() !== headerHtml.trim()) {
          pages.push(currentHtml);
    ...
    // 7) Montar HTML final (tu CSS intacto)
    ...
    <style class="swal2-previsualizacion-a4">
      .report-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;border:4px solid #8493a7ff;padding:5px 10px;border-radius:4px;background:white;}
      .report-header img{max-height:50px;}
      .report-header .header-title{flex:1;text-align:center;font-weight:bold;font-size:14px;}
      .page{position:relative;width:210mm;min-height:297mm;padding:10mm;box-sizing:border-box;background:white;page-break-after:always;}
      .section-block{margin-bottom:1em; /* QUITAR break-inside/page-break-inside */ }
      .tabla-titulo{font-size:12px;font-weight:bold;margin:0 0 .5em; /* QUITAR page-break-inside */ }
      table{width:100%;border-collapse:collapse;margin-bottom:1em; /* QUITAR page-break-inside */ }
      td{border:1px solid #000;padding:.2rem;font-size:12px;}
      .images-wrapper{display:grid;grid-template-columns:repeat(2,1fr);gap:5mm;margin-bottom:1em; /* QUITAR page-break-inside */ }
      .images-wrapper img:only-child{grid-column:1/-1;justify-self:center;}
  ...
  `;
    ...
    // 8) Mostrar SweetAlert con Panzoom y botón PDF
    let zoom: any;
    Swal.fire({
      html,
      width: 'auto',
      ...
      didOpen: () => {
        ...
        btn.onclick = () => {
          const filtered = pages.filter(
            pg => pg.replace(headerHtml, '').replace(/<div style="position:[\s\S]*?<\/div>/, '').trim() !== ''
          );
          const pagesHtmlF = filtered
            .map((pg, idx) => `<div class="page">${pg}<div style="position:absolute;bottom:5mm;right:10mm;font-size:10px;">Hoja ${idx + 1} de ${filtered.length}</div></div>`)
            .join('');
          previewEl.innerHTML = `<style class="swal2-previsualizacion-a4">${styleTag.innerHTML}</style><div id="${previewId}">${pagesHtmlF}</div>`;
          html2pdf()
            .from(previewEl)
            .set({
              margin: [0, 0, 0, 0],
              filename: `${this.informe.titulo}.pdf`,
              html2canvas: { scale: 2, useCORS: true, allowTaint: true, width: previewEl.offsetWidth, windowWidth: previewEl.offsetWidth },
